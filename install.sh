...
function install () {
  ...
  echo "Make sure to add Neovim to your path: export PATH=$PATH:$INSTALL_DIR/$TAR_DIR/bin"
}

if [ -d "$INSTALL_DIR/$TAR_DIR" ]; then
  if yes_or_no "$INSTALL_DIR/$TAR_DIR already exists. Would you like to replace its contents?\n"; then
